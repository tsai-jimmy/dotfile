...
prompt_context() {
  if [ $DEFAULT_USER ]; then
    prompt_segment "$AGNOSTER_CONTEXT_BG" "$AGNOSTER_CONTEXT_FG" "%(!.%{%F{$AGNOSTER_STATUS_ROOT_FG}%}.)$DEFAULT_USER"
  elif [[ "$USER" != "$DEFAULT_USER" || -n "$SSH_CLIENT" ]]; then
    prompt_segment "$AGNOSTER_CONTEXT_BG" "$AGNOSTER_CONTEXT_FG" "%(!.%{%F{$AGNOSTER_STATUS_ROOT_FG}%}.)$USER@%m"
  fi
}
...
prompt_virtualenv() {
  # Conda environment (new logic)
  if [ -n "$CONDA_DEFAULT_ENV" ]; then
    prompt_segment "$AGNOSTER_CONDA_BG" "$AGNOSTER_CONDA_FG" "🐍 $CONDA_DEFAULT_ENV"
  fi
  # Python venv (old logic)
